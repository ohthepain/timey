...
import Vex, { Beam, Stem, Renderer, Stave, StaveNote, EasyScore, Formatter, Articulation, Barline, Modifier, StemmableNote } from "vexflow";
import Flow from "vexflow";

export const PercussionScore: React.FC<{ notes?: string }> = ({ notes = "C#5/q, B4, A4, G#4" }) => {
  ...
  useEffect(() => {
    ...
    const f = new Vex.Factory({
      renderer: { elementId: 'percussionscore', width: containerRef.current.clientWidth, height: 200 },
    });
  ...
  return <div className="h-full w-full bg-orange-400 border-2 border-l-purple-400" ref={containerRef}>PercussionScore</div>;
};
